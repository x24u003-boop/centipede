...
function startRanking() {
    ...
    onValue(q, (snapshot) => {
        ...
        // --- フラグで2つに分離 ---
        const normalList = list.filter(item => item.flag === "normal").slice(0, 10);
        const lastList   = list.filter(item => item.flag === "last").slice(0, 10);

        // --- HTML表示 ---
        const normalDiv = document.getElementById("rankingNormal");
        const lastDiv   = document.getElementById("rankingLast");

        normalDiv.innerHTML = "<b>【通常ステージ ランキング】</b><br>";
        lastDiv.innerHTML   = "<b>【最終ステージ ランキング】</b><br>";

        normalList.forEach((item, i) => {
            const row = document.createElement("div");
            row.textContent = `${i + 1}位 : ${item.name} (${item.score})`;
            normalDiv.appendChild(row);
        });

        lastList.forEach((item, i) => {
            const row = document.createElement("div");
            row.textContent = `${i + 1}位 : ${item.name} (${item.score})`;
